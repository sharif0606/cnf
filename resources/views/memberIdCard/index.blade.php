                    ...
                    <div class="card-body">
                        <form method="GET" action="{{route(currentUser() . '.memberIdCard.index')}}">
                            <div class="row">
                                ...
                                <div class="col-md-3 mb-2">
                                    <input type="text" name="member_serial_no" class="form-control"
                                        placeholder="Member Serial No" value="{{request('member_serial_no')}}">
                                </div>
                                ...
                                <div class="col-md-2 mb-2">
                                    ...
                                    <a href="{{route(currentUser() . '.memberIdCard.index')}}"
                ...
                <div class="card">
                    <div class="table-responsive">
                        <table class="table table-bordered mb-0">
                            <a class="float-end" href="{{route(currentUser() . '.memberIdCard.create')}}"
                            ...
                            <tbody>
                                @forelse($cards as $card)
                                    <tr class="text-center">
                                        <th scope="row">{{ $loop->iteration + ($cards->currentPage() - 1) * $cards->perPage() }}
                                        </th>
                                        ...
                                        <td>{{$card->card_expiration_date ? date('d M Y', strtotime($card->card_expiration_date)) : 'N/A'}}
                                        </td>
                                        <td class="white-space-nowrap">
                                            <a href="{{route(currentUser() . '.memberIdCard.show', encryptor('encrypt', $card->id))}}"
                                                title="View">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            <a href="{{route(currentUser() . '.memberIdCard.edit', encryptor('encrypt', $card->id))}}"
                                                title="Edit">
                                                <i class="bi bi-pencil-square"></i>
                                            </a>
                                            <a class="text-danger" href="javascript:void()"
                                                onclick="$('#form{{$card->id}}').submit()" title="Delete">
                                                <i class="bi bi-trash"></i>
                                            </a>
                                            <form id="form{{$card->id}}" onsubmit="return confirm('Are you sure?')"
                                                action="{{route(currentUser() . '.memberIdCard.destroy', encryptor('encrypt', $card->id))}}"
                                                method="post">
                                                @csrf
                                                @method('delete')
                                            </form>
