                                ...
                                <div class="row mb-3">
                                    ...
                                    <select class="form-control choices form-select" name="member_id" required>
                                        ...
                                        @forelse($ourMember as $d)
                                            <option value="{{$d->id}}" {{ old('member_id')==$d->id?"selected":""}}> {{ $d->name_bn}}--Member No-{{ $d->member_serial_no}}--RSL-{{ $d->renew_serial_no}}</option>
                                        @empty
                                ...
                                <div class="row mb-3">
                                    ...
                                    <select class="form-control form-select" name="session_id" required>
                                        ...
                                        @forelse($comSession as $d)
                                            <option value="{{$d->id}}" {{ old('session_id')==$d->id?"selected":""}}> {{ $d->start_year}}-{{ $d->end_year}}</option>
                                        @empty
                                            <option value="">No Data found</option>
                                        @endforelse
                                    </select>
